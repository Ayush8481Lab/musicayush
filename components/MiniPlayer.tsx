...
export default function MiniPlayer() {
  ...
  const iframeInitialTimeRef = useRef<number>(0); // Locks iframe start time so it never resets
  const videoStartTimeRef = useRef<number>(0);    // Tracks live time for bg sync
  ...
  // Switch to background audio seamlessly when app hides
  useEffect(() => {
  ...
  // RECOMMENDATION ENGINE (Direct Client Fetch, Delayed for Performance)
  useEffect(() => {
    ...
    const fetchRecs = async (vid: string, retries = 5) => {
      ...
      if (isSubscribed && recs.length > 0) {
        ...
        setUpcomingQueue(prev => {
          ...
          const newSongs = formatted.filter((s: any) => !existingIds.has(s.id));
    ...
    if (isAutoPlayContext && upcomingQueue.length <= 3 && !fetchingRecsRef.current) {
      ...
      // Delay Recommendation Fetch to prioritize Audio Streaming
      if (seedVid) {
        recTimer = setTimeout(() => fetchRecs(seedVid as string), 2500);
      }
    }
  ...
  useEffect(() => {
    ...
    // Delay Spotify/Lyrics fetch to ensure Audio starts perfectly
  ...
  // Crisp full-screen lyrics rendering without blur
  const RenderedLyrics = useMemo(() => {
    ...
    return lyrics.map((line: any, idx: number) => {
      ...
      const activeClasses = isLyricsFullScreen 
        ? "text-white text-[36px] font-black drop-shadow-2xl leading-tight opacity-100 translate-y-0" 
        : "text-white text-[28px] font-black drop-shadow-xl leading-tight opacity-100 translate-y-0";
      ...
      const futureClasses = isLyricsFullScreen 
        ? "text-white/30 text-[28px] font-bold hover:text-white/50 leading-tight opacity-30 translate-y-4" 
        : "text-white/30 text-[24px] font-bold hover:text-white/50 leading-tight opacity-30 translate-y-3";
      ...
      return (
        <p 
          ...
          className={`cursor-pointer transition-all duration-500 ease-out origin-left no-select-text transform ${isActive ? activeClasses : isPast ? pastClasses : futureClasses}`}
        >
          {line.words || '♪'}
        </p>
      )
    ...
    return upcomingQueue.map((track: any, index: number) => (
      <div 
        ...
        onClick={() => {
            ...
            setUpcomingQueue((prev: any) => prev.slice(idx + 1));
            setIsPlaying(true);
        }}
        style={{
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* SPOTIFY STYLE LYRIC ENTRY ANIMATION (Smooth Slide Up + Fade) */
      ...
      <div className={`fixed inset-0 z-[99999] text-white transition-all duration-[450ms] ease-[cubic-bezier(0.32,0.72,0,1)] ${isExpanded ? "translate-y-0 opacity-100 overflow-hidden" : "translate-y-full opacity-0 pointer-events-none"}`}>
        ...
        <div className={`absolute inset-0 z-20 overflow-x-hidden scrollbar-hide flex flex-col pointer-events-none ${isLyricsFullScreen ? 'overflow-y-hidden' : 'overflow-y-auto'}`} onScroll={handleScroll}>
          ...
          <div className="w-full flex flex-col flex-shrink-0 pointer-events-auto transition-all duration-500" style={{ height: isLyricsFullScreen ? '100%' : undefined, minHeight: isLyricsFullScreen ? '100%' : '100dvh' }}>
            ...
            <div className={`w-full px-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] mb-2 pt-2 flex flex-col justify-end flex-shrink-0 transition-opacity duration-500 pointer-events-auto`}>
              ...
              {/* SPOTIFY-STYLE MINI LYRIC ANIMATION */}
              <div className={`transition-all duration-500 overflow-hidden w-full relative ${isUiHidden && !isVideoMode ? 'max-h-0 opacity-0 mb-0' : 'mb-2 opacity-100 min-h-[52px] flex items-end'}`}>
                {isLyricsEnabled && !isLyricsFullScreen && syncType === "LINE_SYNCED" && lyrics[activeLyricIndex] && !isVideoMode && (
                  <span 
                     key={`lyric-${activeLyricIndex}`} 
                     className="text-white/95 text-[14px] md:text-[15px] font-bold text-left drop-shadow-md line-clamp-3 leading-snug animate-lyric-enter w-full pr-2 no-select-text absolute bottom-0"
                  >
        ...
        {/* SETTINGS MENU w/ ADVANCED SHARE */}
                   ...
                   <div className="flex flex-col bg-[#1e1e1e] rounded-[16px] overflow-hidden">
                      <button onClick={handleShareSong} className="w-full flex items-center justify-between px-5 py-4 transition-colors active:bg-white/10">
                        <div className="flex flex-col items-start text-left">
                          ...
                          <span className="text-white/50 text-[12px] font-medium mt-0.5">Share exact audio, video, & Spotify links</span>
                        </div>
