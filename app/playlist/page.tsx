...
function PlaylistContent() {
  ...
  useEffect(() => {
    ...
    const fetchPlaylist = async () => {
      try {
        const res = await fetch(`https://ayushm-psi.vercel.app/api/playlists?link=${encodeURIComponent(link)}`);
  ...
  if (!playlist) return <div className="flex h-screen items-center justify-center bg-black text-white">Playlist not found</div>;

  return (
    <div className="pb-24">
      {/* Header */}
      <div className="relative h-72 w-full bg-gradient-to-b from-indigo-900 to-black p-4 pt-12 flex flex-col justify-end">
        ...
        <div className="flex items-end gap-4 z-10">
          <img src={getImageUrl(playlist.image)} className="w-32 h-32 rounded-xl shadow-2xl object-cover" />
          <div className="overflow-hidden">
            <p className="text-xs uppercase tracking-widest text-neutral-400 font-bold mb-1">Playlist</p>
            <h1 className="text-2xl font-black truncate text-white">{playlist.name || playlist.title}</h1>
            <p className="text-xs text-neutral-500 mt-2">{playlist.songCount || playlist.songs?.length} Songs</p>
          </div>
        </div>
      </div>
